...
export function SupervisorModePipelineModal({
  ...
  const stages: WorkflowStage[] = useMemo(() => {
    ...
    // Add Response Streaming stage (combines generation + streaming)
    // Response is already generated by task tools, this just extracts and streams it
    allStages.push({
      id: 'response_streaming_started',
      name: 'Response Delivery',
      description: 'Extracting and streaming final response to client',
      status: getStageStatus('response_streaming_started'),
      icon: <IconSparkles size={20} />,
      color: 'green',
      substages: [
        {
          name: 'Extracting final response from agent output',
  ...
  function getStageStatus(stageId: string): 'pending' | 'active' | 'completed' | 'skipped' {
    ...
    const stageOrder: string[] = [
      ...
      'response_streaming_started',  // Response delivery (extract + stream)
      'workflow_complete'
    ];

...
from src.workflow.retrieval import parallel_retrieval, reciprocal_rank_fusion
...
async def document_retriever(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        if augmented_queries and isinstance(augmented_queries, list):
            ...
            # 3. Sub-queries from decomposition
            elif enhanced_query.get("sub_queries"):
                query_variants = enhanced_query.get("sub_queries")
            # 4. For single-value enhancements (step-back, hyde), create list
            elif enhanced_query.get("step_back_query"):
                query_variants = [enhanced_query.get("step_back_query")]

...
class CreateSessionRequest(BaseModel):
    name: Optional[str] = Field(None, max_length=100, description="Optional name for the conversation")


class RenameSessionRequest(BaseModel):
    new_name: str = Field(..., min_length=1, max_length=100, description="New name for the conversation")
...
async def send_chat_message(conversation_id: str, chat_message: ChatMessage, current_user: User = Depends(get_current_user)):
    """
...
async def reset_conversation_messages(
    ...
    """
    try:
        success = conversation_history_service.reset_conversation_messages(conversation_id, current_user.id)
...
async def get_conversation_sessions(
    ...
    try:
        ...
        for session in sessions:
                    session_responses.append(ConversationSessionResponse(
            id=str(session._id),
...
async def get_conversation_session(
    ...
    """
    Get specific conversation session details and messages.
    """
    try:
        session = conversation_history_service.get_conversation(conversation_id, current_user.id)
        
        if not session:
            raise HTTPException(status_code=404, detail="Conversation session not found")
        
        messages = conversation_history_service.get_session_messages(conversation_id, current_user.id)
...
async def delete_conversation_session(
    ...
    """
    try:
        success = conversation_history_service.delete_conversation(conversation_id, current_user.id)

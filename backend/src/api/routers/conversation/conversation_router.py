...
# ============================================================================
# PROMPT ANALYTICS & ADVANCED FEATURES (Phase 3 - Advanced)
# ============================================================================


@router.get("/{conversation_id}/system-prompts/analytics/overview")
async def get_prompt_analytics(
    conversation_id: str,
    prompt_id: Optional[str] = Query(None, description="Optional specific prompt ID"),
    current_user: User = Depends(get_current_user),
):
    """
    Get analytics for system prompts (Phase 3).

    Returns usage statistics, performance metrics, and comparison data.
    """
    try:
        analytics = conversation_history_service.get_prompt_analytics(
            conversation_id=conversation_id,
            prompt_id=prompt_id,
            user_id=current_user.id,
        )
        ...
        if not analytics:
            raise HTTPException(
                status_code=404,
                detail="Conversation or prompt not found",
            )
...
@router.get("/{conversation_id}/system-prompts/{prompt_id}/history")
async def get_prompt_history(
    ...
    Returns all versions and changes over time.
    """
    try:
        history = conversation_history_service.get_prompt_history(
            conversation_id=conversation_id,
            prompt_id=prompt_id,
            user_id=current_user.id,
        )
...
@router.get("/{conversation_id}/system-prompts/search/by-tag")
async def search_prompts_by_tag(
    ...
    """
    Search system prompts by tag (Phase 3).

    Returns all prompts with the specified tag.
    """
    try:
        prompts = conversation_history_service.search_prompts_by_tag(
            conversation_id=conversation_id,
            tag=tag,
            user_id=current_user.id,
        )
...
class ComparePromptsRequest(BaseModel):
    """Request to compare two prompts."""

    prompt_id_1: str = Field(..., description="First prompt ID")
    prompt_id_2: str = Field(..., description="Second prompt ID")


@router.post("/{conversation_id}/system-prompts/compare")
async def compare_prompts(
    ...
    """
    Compare two system prompts side-by-side (Phase 3).

    Returns differences in content, metadata, and usage statistics.
    """
    try:
        comparison = conversation_history_service.compare_prompts(
            conversation_id=conversation_id,
            prompt_id_1=request.prompt_id_1,
            prompt_id_2=request.prompt_id_2,
            user_id=current_user.id,
        )

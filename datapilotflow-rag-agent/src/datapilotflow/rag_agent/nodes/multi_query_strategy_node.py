...
async def multi_query_strategy_node(state: WorkflowState) -> WorkflowState:
    ...
    """
    logger.debug("Node starting: [NODE START] multi_query_strategy_node")
    ...
    try:
        ...
        logger.debug(f"Multi-Query: Generated {len(variants)} variants from LLM")

        # Update state
        state["enhanced_query"] = {"multi_query_variants": variants}
        state["enhancement_strategies_applied"] = ["multi_query"]
        logger.debug("[NODE FINISH] multi_query_strategy_node")
        return state
        ...
        logger.error(f"[NODE FINISH] multi_query_strategy_node (with error)")
        # Fallback
        ...
        logger.info(f"⏱ Multi-Query node completed in {elapsed:.2f}ms")
...
def _parse_variants(llm_response: str, config: dict[str, Any]) -> list[str]:
    ...
    try:
        ...
        if not isinstance(variants, list):
            logger.warning(f"LLM returned non-array JSON: {type(variants)}")
            return []
        ...
        if variants:
            logger.debug(f"Extracted {len(variants)} query variants from JSON array")
        else:
            logger.warning(f"No valid variants found in JSON array")

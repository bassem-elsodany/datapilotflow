...
You migrate your existing Mule 3 applications that are managed by API Manager
to Mule 4 using the following steps:

  1. Prepare for your migration

Based on how your existing Mule 3 application is configured, carefully review
the prerequisites to prepare for your migration.

  2. Determine your migration approach

Based on several factors, such as whether the migration requires taking
downtime, removing applied policies, migrating contracts, or creating an
instance for the migration.
...
Before you migrate your existing API-managed Mule 3 applications to Mule 4,
carefully review the following guidelines and determine:

  * How your Mule 3 application is managed: basic endpoint, integration application, or complex proxy

To migrate your application in this scenario, first [review how to migrate API
gateways](migration-api-gateways) to prepare, and then [migrate to Mule
4](migration-intro).

  * How your application is secured: default or custom policies

If you applied custom policies to your Mule 3 API, follow [Migrating API
Policies](migration-api-gateways-policies). If you have applied an out-of-the-
box proxy, you can recreate the proxy by using the API Manager UI.

  * Whether you have classified your API

If you have not classified your API, first [classify an API Instance into an
Environment](../../api-manager/latest/classify-api-task).

## Determine Your Migration Approach

You migrate your API Manager-managed Mule 3 application by creating a new API
instance. Before migrating to Mule 4, carefully consider the following factors
to determine the approach that best suits your requirements:
...
Use the high-level proposal illustrated in the following diagram to use a new
API instance for configuring an independent Mule runtime instance on Mule 4:

![A diagram showing migration from Mule 3 to Mule 4 with components like
proxies and load balancers](_images/new-instance-migration-mule4.png)

If your Mule application is not a proxy, this same proposal applies to your
migration procedure, without the backend component.
...
Before migrating your application by creating a new API instance, migrate
existing API contracts to your new Mule 4 environment.
...
If you are not ready to migrate to Mule 4 and you need to migrate to the
latest version of Mule 3, see [Upgrading Mule Runtime Engine(Versions 3.x to
3.n)](../../release-notes/mule-runtime/updating-mule-versions).

Carefully review the release notes for the Mule version to which you are
migrating. When you start the new Mule runtime with the same application
pointing to the same API instance, your existent API is instantly managed with
the latest Mule 3.x version. You do not need to perform any additional steps
in API Manager.

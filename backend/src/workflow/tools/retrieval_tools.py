...
def vector_search(query: str, limit: int = 5):
    ...
    if not query or not query.strip():
        raise InvalidSearchParametersError("vector_search", "query", query, "Query cannot be empty")
    
    if limit <= 0:
        raise InvalidSearchParametersError("vector_search", "limit", limit, "Limit must be positive")
    ...
    try:
        with MilvusClientWrapper(
            ...
            results = milvus_client.search_with_vector(
                ...
                limit=limit,
                return_metadata=["page_content", "title", "knowledge_source", "chunk_id", "entities", "relationships", "tags", "source_url"]
            )
...
@tool(response_format="content_and_artifact")
def semantic_search(query: str, filters: Optional[Dict[str, Any]] = None, limit: int = 5):
    """
    ...
    Raises:
        ...
        WeaviateConnectionError: If Weaviate connection fails
    """
    logger.debug(f"Semantic search: query='{query}', filters='{filters}', limit={limit}")
    
    # Validate parameters
    if not query or not query.strip():
        raise InvalidSearchParametersError("semantic_search", "query", query, "Query cannot be empty")
    
    if limit <= 0:
        raise InvalidSearchParametersError("semantic_search", "limit", limit, "Limit must be positive")
...
def hybrid_search(query: str, alpha: float = 0.5, limit: int = 5):
    ...
    # Validate parameters
    if not query or not query.strip():
        raise InvalidSearchParametersError("hybrid_search", "query", query, "Query cannot be empty")
    
    if limit <= 0:
        raise InvalidSearchParametersError("hybrid_search", "limit", limit, "Limit must be positive")
    
    if not 0.0 <= alpha <= 1.0:
        raise InvalidSearchParametersError("hybrid_search", "alpha", alpha, "Alpha must be between 0.0 and 1.0")
...
def entity_search(entities: List[str], limit: int = 5):
    ...
    if not entities or not isinstance(entities, list) or len(entities) == 0:
        raise InvalidSearchParametersError("entity_search", "entities", entities, "Entities list cannot be empty")
    
    if limit <= 0:
        raise InvalidSearchParametersError("entity_search", "limit", limit, "Limit must be positive")
    
    # Filter out empty entities
    valid_entities = [entity.strip() for entity in entities if entity and entity.strip()]
    if not valid_entities:
        raise InvalidSearchParametersError("entity_search", "entities", entities, "No valid entities provided")
...
def relationship_search(relationships: List[str], limit: int = 5):
    ...
    if not relationships or not isinstance(relationships, list) or len(relationships) == 0:
        raise InvalidSearchParametersError("relationship_search", "relationships", relationships, "Relationships list cannot be empty")
    
    if limit <= 0:
        raise InvalidSearchParametersError("relationship_search", "limit", limit, "Limit must be positive")
    
    # Filter out empty relationships
    valid_relationships = [rel.strip() for rel in relationships if rel and rel.strip()]
    if not valid_relationships:
        raise InvalidSearchParametersError("relationship_search", "relationships", relationships, "No valid relationships provided")
...
@tool(response_format="content_and_artifact")
def knowledge_graph_search(query: str, entities: Optional[List[str]] = None, relationships: Optional[List[str]] = None, limit: int = 5):
    """
    ...
    Raises:
        ...
        WeaviateConnectionError: If Weaviate connection fails
    """
    logger.debug(f"Knowledge graph search: query='{query}', entities='{entities}', relationships='{relationships}', limit={limit}")
    
    # Validate parameters
    if not query or not query.strip():
        raise InvalidSearchParametersError("knowledge_graph_search", "query", query, "Query cannot be empty")
    
    if limit <= 0:
        raise InvalidSearchParametersError("knowledge_graph_search", "limit", limit, "Limit must be positive")
...
def get_document_by_id(document_id: str):
    ...
    if not document_id or not document_id.strip():
        raise InvalidSearchParametersError("get_document_by_id", "document_id", document_id, "Document ID cannot be empty")

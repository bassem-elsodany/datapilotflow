...
same functionality through different attributes or components, or you no
longer need to configure the functionality in Mule 4.
...
modifications:
  ...
  * Removal of the `failureExpression` and use of a Validation processor for Mule 4, instead.
...
requires you to wrap the processors inside a processor chain (`<processor-
chain/>`). In Mule 4, Until Successful supports multiple processors without
the need for a processor chain.
...
The `failureExpression` attribute has been removed. You can replace it with a
Validation processor that sets your verification expression.
...
`RETRY_EXHAUSTED` exceptions and send the message to that endpoint or
processor.
...
The Mule 3 `ackExpression` attribute configures synchronous production of a
response payload. This attribute no longer exists in Mule 4. To achieve this
behavior in your app, you can configure a Set Payload component after Until
Successful.
...
The Mule 4 example sets the `#[message:correlationId]` value through the Set
Payload `<set-payload/>` component.
...
The `synchronous` attribute has been removed because it is no longer needed.
In Mule 4, several changes were introduced to the execution engine. With the
non-blocking architecture, you no longer need it.

## See Also

  * [Introduction to Mule 4: Execution Engine Threads and Concurrency](intro-engine).

  * [Execution Engine](execution-engine)

  * [Until Successful Scope](until-successful-scope)

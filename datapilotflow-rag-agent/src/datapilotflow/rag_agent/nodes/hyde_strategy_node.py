...
async def hyde_strategy_node(state: WorkflowState) -> WorkflowState:
    ...
    """
    logger.debug("Node starting: [NODE START] hyde_strategy_node")
    ...
    try:
        ...
        logger.info(f" HyDE: Generated {len(hypothetical_answer.split())} word answer")

        # Update state
        state["enhanced_query"] = {"hypothetical_answer": hypothetical_answer}
        state["enhancement_strategies_applied"] = ["hyde"]
        logger.debug("[NODE FINISH] hyde_strategy_node")
        return state
        ...
        logger.error(f"[NODE FINISH] hyde_strategy_node (with error)")
        # Fallback
        ...
        logger.info(f"⏱ HyDE node completed in {elapsed:.2f}ms")

...
async def decomposition_strategy_node(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        if sub_queries and len(sub_queries) >= 2:
            ...
            logger.warning(
                f"⚠️  Failed to generate sub-queries, using original query only"
            )
...
def _parse_decomposition_response(response: str, config: Dict[str, Any]) -> List[str]:
    """
    ...
    try:
        ...
        if not isinstance(sub_queries, list):
            logger.warning(f"⚠️  LLM returned non-array JSON: {type(sub_queries)}")
            return []
        ...
        if sub_queries:
            logger.info(f"📝 Extracted {len(sub_queries)} sub-queries from JSON array")
        else:
            logger.warning("⚠️  No valid sub-queries found in JSON array")

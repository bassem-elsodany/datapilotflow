...
from datapilotflow.domain.notification.notification import (
    NotificationCreate,
...
async def create_notification(
    ...
    try:
        ...
        notification_data.user_id = current_user.id or current_user.username
...
async def get_notifications(
    ...
    try:
        ...
        result = service.get_user_notifications(
            user_id=current_user.id or current_user.username,
            limit=limit,
            skip=skip,
            status=status,
...
async def mark_all_notifications_as_read(
    ...
    try:
        service = NotificationService()
        success = service.mark_all_as_read(current_user.id or current_user.username)
...
@router.get(
    "/session/{session_id}/notifications", response_model=List[NotificationResponse]
)
...
async def get_notification_stats(current_user: User = Depends(get_current_user)):
    ...
    try:
        service = NotificationService()
        stats = service.get_notification_stats(current_user.id or current_user.username)
        return stats
...
async def broadcast_notification(notification_data: dict):
    ...
    try:
        from datapilotflow.services.notification.notification_websocket_service import (

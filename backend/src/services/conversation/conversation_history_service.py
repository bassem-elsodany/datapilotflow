...
class ConversationHistoryService:
    ...
    # ============ Prompt Versioning & Analytics (Phase 3 - Advanced) ============

    def get_prompt_history(
        self, conversation_id: str, prompt_id: str, user_id: str = None
    ) -> List[Dict[str, Any]]:
        """
        Get version history for a system prompt (Phase 3).

        Returns a list of all versions of a prompt with timestamps and changes.
        """
        try:
            from bson import ObjectId
            ...
            if (
                not doc
                or "system_prompt_tasks" not in doc
                or len(doc["system_prompt_tasks"]) == 0
            ):
                return []

            prompt = doc["system_prompt_tasks"][0]
            # For now, return the current state with version info
            # Full versioning would require storing historical snapshots
            return [
                {
    ...
    def get_prompt_analytics(
        self, conversation_id: str, prompt_id: str = None, user_id: str = None
    ) -> Dict[str, Any]:
        """
        Get analytics for system prompts (Phase 3).
        ...
        try:
            ...
            prompts = doc["system_prompt_tasks"]

            if prompt_id:
                # Analytics for specific prompt
                prompt = next(
                    (p for p in prompts if p.get("id") == prompt_id), None
                )
                if not prompt:
                    return {}
                ...
                active_prompts = [p for p in prompts if p.get("is_active", True)]
                ...
                return {
                    ...
                    "average_usage": (
                        total_usage / len(prompts) if prompts else 0
                    ),
                    "prompts": [
                        {
                            "id": p.get("id", ""),
            ...
            logger.error(f"Error retrieving prompt analytics: {e}")
            return {}

    def search_prompts_by_tag(
        self, conversation_id: str, tag: str, user_id: str = None
    ) -> List[SystemPromptTask]:
        """
        Search prompts by tag (Phase 3 enhancement).

        Allows filtering prompts by tags for better organization.
        """
        try:
            from bson import ObjectId

            query = {"_id": ObjectId(conversation_id), "system_prompt_tasks.tags": tag}
            if user_id:
                query["user_id"] = user_id
            ...
            matching_prompts = [
                p
                for p in doc["system_prompt_tasks"]
                if tag in p.get("tags", [])
            ]

            return [self._dict_to_system_prompt(p) for p in matching_prompts]

        except Exception as e:
            logger.error(f"Error searching prompts by tag: {e}")
            return []
    ...
    def compare_prompts(
        ...
        """
        Compare two prompts side-by-side (Phase 3 feature).

        Useful for understanding differences between prompt versions.
        """
        try:
            prompt1 = self.get_system_prompt(
                conversation_id, prompt_id_1, user_id
            )
            prompt2 = self.get_system_prompt(
                conversation_id, prompt_id_2, user_id
            )
            ...
            return {
                "prompt_1": {
                    ...
                    "usage_count": prompt1.usage_count,
                ...
                "prompt_2": {
                    ...
                    "usage_count": prompt2.usage_count,
                    "created_at": prompt2.created_at,
                },
                "differences": {
                    "name_differs": prompt1.name != prompt2.name,
                    "description_differs": prompt1.description != prompt2.description,
                    "prompt_differs": prompt1.system_prompt != prompt2.system_prompt,
                    "tags_differ": set(prompt1.tags) != set(prompt2.tags),
                    "version_diff": prompt2.version - prompt1.version,
                    "usage_diff": prompt2.usage_count - prompt1.usage_count,
                },
            }

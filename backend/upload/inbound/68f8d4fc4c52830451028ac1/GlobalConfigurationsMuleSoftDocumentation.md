...
Global configurations for a mule application such as the default transaction
timeout or default error handler can be configured in the `<configuration>`
element of the root level of the [Mule Configuration File](about-mule-
configuration).  
...
![A configuration panel for creating global settings with fields for HTTP
listener configuration](_images/mruntime-global-settings-create.png)

  2. Select **Global Configurations** > **Configuration** :

![A menu for choosing types of global configurations including caching and
scheduler settings](_images/mruntime-global-configurations.png)

  3. Configure the **Default Error Handler** and the **HA Profile** values.

## Create a Global Configuration by Editing the XML

The following example defines a global error handler, and then a global
configuration that specifies a default transaction timeout and references the
global error handler as the default:
...
`defaultErrorHandler-ref` | `string` | no | none | The default error handler for every flow. This must be a reference to a global error handler.  
...
For configuration details, see: [Dynamic Configuration Expiration](../../mule-
sdk/latest/static-dynamic-configs#dynamic-config-expiration).  
`cluster-config` | Optional. Mule application cluster configuration.  
For configuration details, see: [Clustering for High Performance](mule-high-
availability-ha-clusters#cluster-high-performance).  
  
## See Also

  * [Error Handling](error-handling)

  * [Dynamic Configurations](../../mule-sdk/latest/static-dynamic-configs)

  * [Mule High Availability Cluster](mule-high-availability-ha-clusters)
